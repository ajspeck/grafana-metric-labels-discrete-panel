...
class DiscretePanelCtrl extends CanvasPanelCtrl {
  ...
  onInitEditMode() {
    ...
    this.addEditorTab(
      'Options',
      'public/plugins/metrics-discrete-panel/partials/editor.options.html',
      1
    );
    this.addEditorTab(
      'Legend',
      'public/plugins/metrics-discrete-panel/partials/editor.legend.html',
      3
    );
    this.addEditorTab(
      'Colors',
      'public/plugins/metrics-discrete-panel/partials/editor.colors.html',
      4
    );
    this.addEditorTab(
      'Mappings',
      'public/plugins/metrics-discrete-panel/partials/editor.mappings.html',
      5
    );
    ...
    _.forEach(dataList, metric => {
      if ('table' === metric.type) {
        ...
        for (let i = 1; i < metric.columns.length; i++) {
          if (this.panel.valueColumnName)
          {
            if (metric.columns[i].text!=this.panel.valueColumnName)
            {
              continue;
            }
          }
          var arr:{ time: number, val: string }[] = [];
          for (let j = 0; j < metric.rows.length; j++) {
            const row = metric.rows[j];
            arr.push({time:row[0], val:this.formatValue(row[i])});
          }
          let sortedArray = arr.sort((n1,n2) => n1.time - n2.time);
          const res = new DistinctPoints(metric.columns[i].text);
          for (let j = 0; j < sortedArray.length; j++) {
            const row = sortedArray[j];
            res.add(row.time, row.val);
          }
          res.finish(this);
          data.push(res);
        }
